...
- 実装: `pull_request`（main、opened/synchronize/reopened/ready_for_review）と`workflow_dispatch`だけをtriggerにする。draft PRはrunnerを起動しない。`contents: read`だけを付与し、PRごとのconcurrencyで古い実行をcancelする。Ubuntu最新runner、15分timeout、Node 22、npm 11.9.0、`npm ci`、`npm run verify`を順に実行し、Next telemetryを無効化する。checkoutはcredentialsを保持せず、履歴を1にする。
