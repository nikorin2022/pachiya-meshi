...
S1-08 で、ローカル手動実行用として次のnpm scriptsを追加した。定期実行ではなく、この時点で自動commit・push・deployは行わない。
...
| `verify` | 生成、両型チェック、監査テスト、書込みなし監査、本番buildを順に実行する | いずれかの工程の失敗 |

`verify` は `generate:halls`、`typecheck:scripts`、`typecheck`、`test:audit`、`audit:data:no-write`、`build:app` の順に実行する。`build:app` は既存`build`本体と同じ `next build` を直接実行するため、verify内での `generate:halls` は1回だけである。通常の `npm run build` は既存の `prebuild` 契約を維持し、引き続き `generate:halls` の後にbuildする。`warning` だけの監査は終了コード0のため継続する一方、error/critical、型エラー、テスト失敗、build失敗では停止する。Google Fonts障害など外部要因でもbuildが失敗すればverifyは失敗する。`audit:data` のJSON出力先 `artifacts/automation/data-audit-report.json` はgitignore済みである。
