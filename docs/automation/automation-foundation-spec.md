...
- 文書状態: Phase 0・Phase 1の読み取り専用監査基盤はS1-09まで実装・コミット済み。S1-10のGitHub Actions設定はローカル実装済みで、GitHubへは未pushのため未実行・未有効化
- 調査日: 2026-07-12
- 対象リポジトリ: `nikorin2022/pachiya-meshi`
- 対象ブランチ: `main`
- 今回の実装範囲: 読み取り専用監査、ローカル品質ゲート、未pushのGitHub Actions設定まで。自動公開、自動修正、自動merge、deployは未実装
...
- GitHub Actionsの標準job logによる実行結果確認。S1-10ではartifactを作成・保存しない
...
- CI: S1-10ではGitHub Actionsの標準job logだけを使用し、artifactの作成・アップロード・保存は行わない。Actionsストレージと運用負荷を増やさないため、artifactは将来、必要性と費用を別途承認した場合だけ検討する
...
最小構成は、Node.js、TypeScript/tsx、Zod、npm scripts、GitHub Actions、既存Vercelだけとする。新規DB、キュー、監視SaaS、有料SEOツールを導入しない。状態はGit管理された設定、ローカルのgitignore対象レポート、GitHub Actionsの実行履歴・job logで管理し、Phase 3まで外部APIを使わない。
...
S1-09 時点で、ローカル手動実行用として次のnpm scriptsを追加した。定期実行ではなく、この時点で自動commit・push・deployは行わない。

| script | 役割 | 失敗条件 |
| --- | --- | --- |
| `audit:data` | 正本監査を実行してJSONレポートを書き出す | critical/error |
| `audit:data:no-write` | レポートを書き出さず正本監査を実行する | critical/error |
| `audit:site` | build成果物のサイト構造監査を実行してJSONレポートを書き出す | critical/error |
| `audit:site:no-write` | レポートを書き出さずbuild成果物のサイト構造監査を実行する | critical/error |
| `test:audit` | S1-07の監査fixture・回帰テストを実行する | テスト失敗 |
| `test:audit:site` | S1-09のサイト監査fixture・回帰テストを実行する | テスト失敗 |
| `typecheck` | アプリ用root TypeScript設定を検査する | 型エラー |
| `typecheck:scripts` | `scripts/tsconfig.json` を検査する | 型エラー |
| `build:app` | `prebuild`を実行せず、既存`build`本体と同じproduction buildを実行する | build失敗 |
| `verify` | 生成、両型チェック、両監査テスト、データ監査、本番build、サイト監査を順に実行する | いずれかの工程の失敗 |

`verify` は `generate:halls`、`typecheck:scripts`、`typecheck`、`test:audit`、`test:audit:site`、`audit:data:no-write`、`build:app`、`audit:site:no-write` の順に実行する。サイト監査はbuild後のroute manifest、sitemap、静的HTMLを読むため、必ず `build:app` の後に実行する。`build:app` は既存`build`本体と同じ `next build` を直接実行するため、verify内での `generate:halls` は1回だけである。通常の `npm run build` は既存の `prebuild` 契約を維持し、引き続き `generate:halls` の後にbuildする。`warning` だけの監査は終了コード0のため継続する一方、error/critical、型エラー、テスト失敗、build失敗では停止する。Google Fonts障害など外部要因でもbuildが失敗すればverifyは失敗する。`audit:data` と `audit:site` のJSON出力先はともにgitignore済みの `artifacts/automation/` 配下である。
...
2. Node 22設定
3. npm 11.9.0設定
4. Node/npm version表示
5. `npm ci`
6. `npm run verify`

`npm run verify`内部の順序は7.1のとおり、`generate:halls`、`typecheck:scripts`、`typecheck`、`test:audit`、`test:audit:site`、`audit:data:no-write`、`build:app`、`audit:site:no-write`である。S1-10では生成物差分専用の`git diff --exit-code`工程、個別の`audit:data`・`build`・`audit:site`、JSONレポートのartifact保存は行わない。生成物差分の専用ゲートは将来課題として別途検討する。
...
- 構造化された監査結果とGitHub Actions標準job log。S1-10ではartifactを使用しない
...
- 変更対象: `package.json`、本仕様書。
- 実装: `audit:data`、`audit:data:no-write`、`test:audit`、`typecheck`、`typecheck:scripts`、`build:app`、`verify`。`build:app` は既存 `build` 本体と同じ `next build` を実行し、verify内の生成を1回にする。既存 `build` / `prebuild` / `generate:halls` は変更しない。`audit:site` はS1-09まで追加しない。
- テスト: Windowsでは `.cmd` 経由、CIでは通常npmで全コマンド実行。
- 完了条件: `verify` が生成、scripts型チェック、アプリ型チェック、監査テスト、書込みなし監査、`build:app`の順に停止し、warningだけでは継続する。これはローカル手動実行用であり、定期実行、自動commit、push、deployを行わない。
- 依存: S1-06、S1-07。
...
- 変更対象: `scripts/audit/site/**`、`package.json`、本仕様書。
- 実装: build後の `.next` route manifest、sitemap XML、静的HTMLを読取り専用で収集し、sitemap URL、内部リンク、JSON-LD、noindex除外、URL件数baselineを検査する。manifestは監査に必要な文字列route mapとして厳格に検証する。sitemapは固定ページ、飲食店を持つ生成hall、全source hallから得るarea/chain、guideを入力から再構成した期待集合と完全一致させる。runtime routeはprerender結果と分離して検査し、動的route templateだけでは内部リンクを許可しない。内部リンクは各文書routeを基準に解決し、同一originだけを検査する。JSON-LDは型に応じた識別URLとBreadcrumbのURL値を検証する。baselineは `scripts/audit/site/site-url-baseline.json` にレビュー可能な固定値として置く。サイト監査は既存の50データルール、version、inputHashを変更しない。
- テスト: 正常、URL形式・重複・外部host・未定義ID・noindex、必須/想定外sitemap URL、必須runtime route、壊れたJSON-LD、canonical JSON-LD重複、JSON-LD外部URLと識別URL、文書route基準の内部リンク、baseline差分、manifest不在/不正、全12通常ルール発火、system criticalの情報境界、CLI、Windows fallback/rollback・cleanupを含むレポート書込み、全スナップショット配列の逆順安定性。
- 完了条件: 現行build成果物でsite error/critical 0となり、異常fixtureで失敗する。`verify`はサイト監査テストをbuild前、書込みなしサイト監査をbuild後に実行する。
- 依存: S1-08。
- リスク: Next.js内部成果物へ過度に依存する。
- 誤検知: 動的 `/search`、クエリ、アンカー、末尾スラッシュ。
- Codex注意: normal rule codeとsystem critical codeを分離し、system codeはcheckedRulesに含めない。ローカル手動実行だけであり、GitHub Actions、cron、schedule、commit、push、deploy、外部APIを有効化しない。

### S1-10 GitHub Actions

- 目的: mainを対象とするPull Requestと手動実行で、ローカルの`npm run verify`と同じ読み取り専用品質ゲートを実行する。通常push、定期実行、main上の直接実行は行わない。
- 変更対象: `.github/workflows/verify.yml`、本仕様書。
- 実装: `pull_request`（main、opened/synchronize/reopened/ready_for_review）と`workflow_dispatch`だけをtriggerにする。draft PRはrunnerを起動しない。`contents: read`だけを付与し、PRごとのconcurrencyで古い実行をcancelする。Ubuntu最新runner、15分timeout、ActionsのNode 24ランタイムを使用するcheckout@v5/setup-node@v5、アプリ実行Node 22、npm 11.9.0、`npm ci`、`npm run verify`を順に実行し、Next telemetryを無効化する。setup-nodeの`package-manager-cache: false`でキャッシュを無効化し、checkoutはcredentialsを保持せず、履歴を1にする。
- テスト: workflow本文の禁止語・必須語をローカルで検査し、`npm run verify`を通常環境で実行する。GitHubへのpush前のため、GitHub Actions上の実行結果は未確認と明記する。
- 完了条件: critical/error、型エラー、テスト失敗、build失敗ではjobが失敗し、warningだけなら成功する。artifact、cache、secrets、environment、外部API、deploy、commit、push、mergeを行わない。
- 依存: S1-08、S1-09。
- リスク: 無料枠消費、Google Fontsネットワーク、長時間化。
- 誤検知: 外部ネットワーク障害。
- Codex注意: `pull_request_target`、push/schedule、write権限、cache、artifactを追加しない。workflowファイルをpushしない限りGitHub Actionsは有効化されない。
